...
class ASMethods:
    ...
    _numberOfTurns = 0
    _quadrantNumber = 0
    _prevQuadrantNumber = -1

    def __init__(self, startAngle):
        """ Input Raw start angle """
        self._startAngle = self.toRawDeg(startAngle)

    def toRawDeg(self, angle):
        """ From raw angle (360/4096) = 0.087890625 to raw degree"""
        return angle * 0.087890625

    def toDeg(self, angle):
        """ From raw angle (360/4096) = 0.087890625 to raw degree"""
        degAngle = self.toRawDeg(angle)
        corrAngle = degAngle - self._startAngle

        if corrAngle < 0: corrAngle += 360
        return corrAngle

    def checkQuadrant(self, correctedAngle):
        ...
        # Quadrant 1 0 - 90
        if 0 <= correctedAngle <= 90:
            quadrantNum = 1
        elif 90 < correctedAngle <= 180:
            quadrantNum = 2
        elif 180 < correctedAngle <= 270:
            quadrantNum = 3
        elif 270 < correctedAngle < 360:
            quadrantNum = 4

        if quadrantNum != self._prevQuadrantNumber:
            if quadrantNum == 1 and self._prevQuadrantNumber == 2:
                self._numberOfTurns -= 1/4 # CCW
            elif quadrantNum == 2 and self._prevQuadrantNumber == 1:
                self._numberOfTurns += 1/4

            if quadrantNum == 2 and self._prevQuadrantNumber == 3:
                self._numberOfTurns -= 1/4
            elif quadrantNum == 3 and self._prevQuadrantNumber == 2:
                self._numberOfTurns += 1/4

            if quadrantNum == 3 and self._prevQuadrantNumber == 4:
                self._numberOfTurns -= 1/4
            elif quadrantNum == 4 and self._prevQuadrantNumber == 3:
                self._numberOfTurns += 1/4

            if quadrantNum == 4 and self._prevQuadrantNumber == 1:
                self._numberOfTurns -= 1/4
            elif quadrantNum == 1 and self._prevQuadrantNumber == 4:
                self._numberOfTurns += 1/4
