...
def main(args: List[str], **kwargs) -> None:
    ...
    root = opts['PATH']
    python_files = kwargs['python_files']
    print(python_files)
    chdir = opts['--chdir']
    if chdir:
        os.chdir(chdir)
    ...
    if os.path.isdir(root):
        ...
        to_upload = [python_files]
    print(rel_root)

    created_cache = set()
    for paths in progress('Uploading files', to_upload):
        for path in paths:
...
def progress(msg: str, xs: list[str]) -> Iterable[T]:
...
if __name__ == '__main__':
    python_files = [file for file in os.listdir(".") if file.endswith('.py') and file != "upload.py"]
    # port = sys.argv[1]
    # arr.insert(2, python_files)
    # print()
    # for x in python_files:
    main(sys.argv[1:], python_files=python_files)
